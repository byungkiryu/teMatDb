...
if (1):    
    ...
    ax.plot( Temp, ZT1, label='ZT (figure)')
    ax.plot( Temp, ZT2, label='ZT (TEP)')
...
if (1):    
    ...
    diag = [0.4, 1.3]
    ax.scatter( ZT2, ZT1, s=20, facecolor='None', edgecolor='C0', label='ZT-ZT' )
    ax.plot( diag, diag, color='black', linestyle='dashed', alpha=0.5)
        
    ax.set_ylabel('Raw ZT from figure')
    ax.set_xlabel('Reevaluated ZT from TEP')
