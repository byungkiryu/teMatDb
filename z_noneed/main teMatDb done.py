...
st.markdown("- High quality thermoelectric (TE) database (DB), teMatDb (ver1.1.6)")
...
df_tematdb_meta             = load_excel( file_tematdb_metadata_csv, 'list')
...
with st.sidebar:
    ...
    if db_mode == 'disabled':
        st.warning("`teMatDb_expt` is currently not available.")
        st.stop()  # 선택되었을 경우 이후 코드 실행 방지
        
    
    if (db_mode == 'teMatDb'):
        
        # df_tematdb_meta = pd.read_excel("./"+file_tematdb_metadata_csv, sheet_name='list', )
