...
class Device:
    ...
    def fast_run_with_max_efficiency(self, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, quiet=True):
        ...
        self.run_with_given_I(I=I_next, given_no_loop=given_no_loop, max_no_loop=max_no_loop, abs_tol=abs_tol, quiet=quiet)
        self.I = I_next
    ...
    def run_with_max_efficiency(self, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, quiet=True):
        ...
        self.I = I_next
        self.run_with_given_I(I=self.I, given_no_loop=given_no_loop, max_no_loop=max_no_loop, abs_tol=abs_tol, quiet=quiet)
    ...
    def run_with_max_power(self, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, quiet=True):
        self.run_with_given_gamma(gamma=1, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, quiet=True)
    ...
    def run_with_given_I(self, I, given_no_loop=None, max_no_loop = 100, abs_tol=1e-6, quiet=True):
        for idx, leg in enumerate(self.legs):
            if leg.pn_type == 'n':
                convergent = leg.steadystate.solve(given_no_loop=given_no_loop, max_no_loop = max_no_loop, abs_tol=abs_tol, given_I=-I, quiet=quiet)
            else:
                convergent = leg.steadystate.solve(given_no_loop=given_no_loop, max_no_loop = max_no_loop, abs_tol=abs_tol, given_I=I, quiet=quiet)
            if not convergent:
                warnings.warn("%dth leg did NOT converge." % idx)
        self.I = I
