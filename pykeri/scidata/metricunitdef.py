...
class MetricNonprefix:
    ...
    @lru_cache()
    def name(symbol):
        return MetricNonprefix._all.loc[symbol, MetricNonprefix._NAME]
    @lru_cache()
    def names():
    ...
    @lru_cache()
    def measure(symbol):
        return MetricNonprefix._all.loc[symbol, MetricNonprefix._MEASURE]
    @lru_cache()
    def dimension_symbol(symbol):
        ...
        for key in base_dic.keys():
            ...
            if key == 'kg':
                key = 'g'
            if exponent == 1:
                symbol += ' ' + MetricNonprefix._all.loc[key, MetricNonprefix._DIMENSION_SYMBOL]
            else:
                symbol += ' ' + MetricNonprefix._all.loc[key, MetricNonprefix._DIMENSION_SYMBOL] + '^' + str(exponent)
        if len(symbol) > 0:
            symbol = symbol[1:]
        return symbol
    @lru_cache()
    def equivalent(symbol):
        ...
        """
        return (MetricNonprefix._all.loc[symbol, MetricNonprefix._EQUIVALENT], MetricNonprefix._all.loc[symbol, MetricNonprefix._CONVERSION_FACTOR])
...
class MetricPrefix:
    ...
    @lru_cache()
    def name(symbol):
        return MetricPrefix._all.loc[symbol, MetricPrefix._NAME]    
    @lru_cache()
    def conversion_factor(symbol):
        return MetricPrefix._all.loc[symbol, MetricPrefix._CONVERSION_FACTOR]
    @lru_cache()
    def names():
